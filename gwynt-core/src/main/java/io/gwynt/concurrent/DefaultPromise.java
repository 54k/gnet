...
public class DefaultPromise<V> extends AbstractFuture<V> implements Promise<V> {
    ...
    private volatile io.gwynt.concurrent.Promise<V> chainedPromise;
    ...
    private static <V> void successChainedPromise(V result, io.gwynt.concurrent.Promise<V> chainedPromise) {
        chainedPromise.trySuccess(result);
    }

    private static void failChainedPromise(Throwable error, io.gwynt.concurrent.Promise<?> chainedPromise) {
        chainedPromise.tryFailure(error);
    }

    private static void cancelChainedPromise(io.gwynt.concurrent.Promise<?> chainedPromise) {
    ...
    @Override
    public io.gwynt.concurrent.Promise<V> chainPromise(io.gwynt.concurrent.Promise<V> promise) {
        if (promise == null) {
    ...
    @Override
    public io.gwynt.concurrent.Promise<V> chainPromises(io.gwynt.concurrent.Promise<V>... promises) {
        if (promises == null) {
            throw new IllegalArgumentException("promises");
        }
        for (io.gwynt.concurrent.Promise<V> p : promises) {
            chainPromise(p);
        }
        return this;
    }

    private void chainPromise0(io.gwynt.concurrent.Promise<V> promise) {
    ...
    @Override
    public io.gwynt.concurrent.Future<V> addListener(FutureListener<? extends io.gwynt.concurrent.Future<? super V>> futureListener) {
    ...
    @Override
    public io.gwynt.concurrent.Future<V> addListeners(FutureListener<? extends io.gwynt.concurrent.Future<? super V>>... futureListeners) {
        if (futureListeners == null) {
            throw new IllegalArgumentException("futureListeners");
        }

        for (FutureListener<? extends io.gwynt.concurrent.Future<? super V>> l : futureListeners) {
            addListener(l);
        }
        return this;
    }
    ...
    @Override
    public io.gwynt.concurrent.Future<V> removeListener(FutureListener<? extends io.gwynt.concurrent.Future<? super V>> futureListener) {
        if (futureListener == null) {
            throw new IllegalArgumentException("futureListener");
        }
    ...
    @Override
    public io.gwynt.concurrent.Future<V> removeListeners(FutureListener<? extends io.gwynt.concurrent.Future<? super V>>... futureListeners) {
        if (futureListeners == null) {
            throw new IllegalArgumentException("futureListeners");
        }
    ...
    protected void notifyListener(final FutureListener futureListener) {
        if (executor().inExecutorThread()) {
            ...
            invokeLater(() -> futureListener.onComplete(DefaultPromise.this));
        }
    }
    ...
    private void notifyAllListeners() {
        if (!inNotify.compareAndSet(false, true)) {
            invokeLater(this::notifyAllListeners);
        } else {
            notifyListeners();
    ...
    private void notifyChainedPromise() {
        ...
        final io.gwynt.concurrent.Promise<V> promise = chainedPromise;

        if (isSuccess()) {
            if (executor().inExecutorThread()) {
                successChainedPromise((V) res, promise);
            } else {
                invokeLater(() -> successChainedPromise((V) res, promise));
            }
        } else if (isCancelled()) {
            if (executor().inExecutorThread()) {
                cancelChainedPromise(promise);
            } else {
                invokeLater(() -> cancelChainedPromise(promise));
            }
        } else {
            if (executor().inExecutorThread()) {
                failChainedPromise(getCause(), promise);
            } else {
                invokeLater(() -> failChainedPromise(getCause(), promise));
            }
        }
    ...
    @Override
    public io.gwynt.concurrent.Promise<V> setSuccess(V result) {
        if (!done.compareAndSet(false, true)) {
            throw new IllegalStateException("Promise already completed");
        }
        this.result = result;
